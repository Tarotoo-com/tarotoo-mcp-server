...
server.registerTool(
  "get_card_meaning",
  {
    ...
    inputSchema: { name: z.string().describe('Card name, e.g. "The Fool" or "Ace of Cups"') },
  },
  async ({ name }) => {
    const card = findCard(name);
    if (!card) return errorText(`Unknown card: "${name}". Use list_cards to see all 78 valid names.`);
    return text(card);
  }
);
...
server.registerTool(
  "list_cards",
  {
    ...
    inputSchema: {
      arcana: z.enum(["major", "minor"]).optional().describe("Filter by arcana"),
      suit: z.enum(["wands", "cups", "swords", "pentacles"]).optional().describe("Filter by suit (minor arcana only)"),
    },
  },
  async ({ arcana, suit }) => {
    let result = cards;
...
server.registerTool(
  "search_cards",
  {
    ...
    inputSchema: {
      ...
      limit: z.number().int().min(1).max(78).optional().describe("Max results (default 10)"),
    },
  },
  async ({ query, limit }) => {
...
server.registerTool(
  ...
  async ({ name }) => {
    const card = findCard(name);
    if (!card) return errorText(`Unknown card: "${name}". Use list_cards to see all 78 valid names.`);
    return text({ name: card.name, answer: card.yes_no, meaning_upright: card.meaning_upright });
  }
);
...
server.registerTool(
  "draw_cards",
  {
    ...
    inputSchema: {
      count: z.number().int().min(1).max(10).optional().describe("Number of cards to draw (default 1)"),
      allow_reversed: z.boolean().optional().describe("Whether cards may be drawn reversed (default false)"),
    },
  },
